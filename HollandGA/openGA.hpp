...
struct GenerationType
{
	...
	std::vector<thisChromosomeType> chromosomes;
...
public:
	...
	std::vector<GeneType> user_initial_solutions;
	...
	std::function<void(GeneType&,const std::function<double(void)> &rnd01)> init_genes;
	std::function<bool(const GeneType&,MiddleCostType&)> eval_solution;
	std::function<bool(const GeneType&,MiddleCostType&,const thisGenerationType&)> eval_solution_IGA;
	std::function<GeneType(const GeneType&,const std::function<double(void)> &rnd01,double shrink_scale)> mutate;
	std::function<GeneType(const GeneType&,const GeneType&,const std::function<double(void)> &rnd01)> crossover;
	...
	std::function<double(int,const std::function<double(void)> &rnd01)> get_shrink_scale;
	...
	void calculate_N_robj(const thisGenerationType &g)
	{
		if(!g.chromosomes.size())
			throw std::runtime_error("Code should not reach here. A87946516564");
		...
		if(!N_robj)
			throw std::runtime_error("Number of the reduced objective is zero");
	}
	...
	void solve_init()
	{
		...
		if(!is_single_objective())
		{
			...
			if(!reference_vector_divisions)
			{
				...
				{
					std::cout<<"**************************************"<<std::endl;
					std::cout<<"reference_vector_divisions: "<<reference_vector_divisions<<std::endl;
					std::cout<<"**************************************"<<std::endl;
				}
			}
		}
		rank_population(generation0); // used for ellite transfer, crossover and mutation
...
protected:
	...
	void check_settings()
	{
		if(is_interactive())
		{
			...
			if(eval_solution_IGA==nullptr)
				throw std::runtime_error("eval_solution_IGA is null in interactive mode!");
			if(eval_solution!=nullptr)
				throw std::runtime_error("eval_solution is not null in interactive mode (use eval_solution_IGA instead)!");
		}
		else
		{
			if(calculate_IGA_total_fitness!=nullptr)
			...
			if(eval_solution_IGA!=nullptr)
				throw std::runtime_error("eval_solution_IGA is not null in non-interactive mode!");
			if(eval_solution==nullptr)
	...
	void select_population_MO(const thisGenerationType &g,thisGenerationType &g2)
	{
		...
		if(!N_robj)
			throw std::runtime_error("Number of the reduced objectives is zero.");
		...
		Matrix &distances)
	{
		...
		for(unsigned int i=0;i<N_x;i++)
		{
			...
			for(unsigned int j=0;j<N_ref;j++)
			{
				...
				assert(w.size()==norm_obj.size() && "Vector size mismatch!");
	...
	std::vector<std::vector<double>> generate_integerReferenceVectors(int dept,int N_division)
	{
		if(dept<1)
	...
	****************************************************/
	...
	void dynamic_thread_action(
		...
		unsigned int N_add, unsigned int &total_attempts)
	{
		std::vector<std::atomic<bool>> active_threads(N_threads);
	...
	****************************************************/
	...
	void crossover_and_mutation(thisGenerationType &new_generation)
	{
		...
		if(verbose)
		{
			std::cout<<"Mutations and crossovers of "<<N_add<<" solutions are calculated with "<<total_attempts<<" attemps."<<std::endl;
		}
	}

	StopReason stop_criteria()
	{
